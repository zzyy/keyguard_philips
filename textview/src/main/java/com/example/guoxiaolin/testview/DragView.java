...
public class DragView extends LinearLayout {
    ...
    public DragView(Context context, AttributeSet attrs) {
        ...
        mScreenHeight = outMetrics.heightPixels / 3;
        ContentHeight=mScreenHeight;
        HeaderHeight=mScreenHeight;

        // ?????
        mScroller = new Scroller(context);
        mScroller.startScroll(0, 0, 0, HeaderHeight);// ?????????
    }
...
//                int dy = mLastY - y;
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        if (changed) {
            ...
            lp.height = ContentHeight*childCount;
            setLayoutParams(lp);
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (child.getVisibility() != View.GONE) {
                    if(i==0){
                    child.layout(left, i * HeaderHeight, right, (i + 1)
                            * HeaderHeight);// ?????????????layout

                 }
                    if(i==1){
                        child.layout(left, i * HeaderHeight, right, HeaderHeight+ContentHeight);// ?????????????layout
